...
function CSVer(filename, opt) {
    ...
    if (config && typeof config === 'string' && fs.existsSync(config)) {
        config = require(Path.join(execPath, config));
    } else {
    ...
    var orderMap = config.fileds_order_map[type] || [];
    ...
    return function(data, callback) {
        ...
        for (var i in queryMap) {
            if (queryMap.hasOwnProperty(i)) {
                fileds[queryMap[i].filed] = getValue(query[i]);
            }
        }

        var result = orderMap.map(function(key) {
            return fileds[key] ? fileds[key] : getValue();
        }).join(',') + '\n';
        if (('view' in options)) {
